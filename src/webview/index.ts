...
function overleafPayload(payload: Record<string, unknown> = {}): Record<string, unknown> {
...
const HOMEWORK_INPUTS: Record<string, string> = {
...
/** Same rule homework.sty enforces: an integer, or a dotted run of integers. */
const HOMEWORK_PREFIX_PATTERN = /^-?\d+(\.\d+)*$/;

function homeworkNumeral(value: number, style: string): string {
  const alphabet = (upper: boolean): string => {
    // Mirrors \alph: only a..z exist, and LaTeX errors past that, so stop at z.
    const letter = String.fromCharCode(96 + Math.min(Math.max(value, 1), 26));
    return upper ? letter.toUpperCase() : letter;
  };
  const roman = (upper: boolean): string => {
    const table: Array<[number, string]> = [[10, "x"], [9, "ix"], [5, "v"], [4, "iv"], [1, "i"]];
    let rest = value;
    let out = "";
    for (const [weight, glyph] of table) while (rest >= weight) { out += glyph; rest -= weight; }
    return upper ? out.toUpperCase() : out;
  };
  switch (style) {
    case "alph": return alphabet(false);
    case "Alph": return alphabet(true);
    case "roman": return roman(false);
    case "Roman": return roman(true);
    default: return String(value);
  }
}

/** Renders the first few headings so the effect of the settings is visible up front. */
function renderHomeworkPreview(settings: any): void {
  const prefix = HOMEWORK_PREFIX_PATTERN.test(String(settings.problemPrefix || "")) ? `${settings.problemPrefix}.` : "";
  const problem = (n: number) => `${prefix}${homeworkNumeral(n, settings.problemStyle)}`;
...
function readHomeworkForm(): Record<string, unknown> {
  const settings: Record<string, unknown> = {};
  for (const [key, id] of Object.entries(HOMEWORK_INPUTS)) settings[key] = byId<HTMLInputElement>(id).value;
  settings.problemStyle = byId<HTMLSelectElement>("homeworkProblemStyleSelect").value;
  settings.sectionStyle = byId<HTMLSelectElement>("homeworkSectionStyleSelect").value;
  settings.sectionMode = byId<HTMLSelectElement>("homeworkSectionModeSelect").value;
  settings.problemPrefix = byId<HTMLInputElement>("homeworkProblemPrefixInput").value.trim();
  return settings;
}

/** A prefix that is not a dotted integer is a LaTeX error, so block the save instead. */
function validateHomeworkForm(): boolean {
  const prefix = byId<HTMLInputElement>("homeworkProblemPrefixInput").value.trim();
...
function renderHomeworkSettings(): void {
  ...
  // An older target defines the environments itself; the package cannot load alongside
  // them, so offering to add the hooks would just break the build.
  const inlineMachinery = Boolean(model.state.homework_machinery_inline);
  byId("homeworkHookNotice").textContent = inlineMachinery
    ? "This target still defines the homework environments itself, from a starter generated before the homework package existed. Regenerate it from Project Setup with overwrite enabled to pick up the configurable numbering."
    : "This target does not load the homework package. Enable the hook before changing generated homework settings.";
  byId("homeworkHookNotice").hidden = hooksEnabled;
  byId<HTMLButtonElement>("enableHomeworkHooksBtn").hidden = hooksEnabled || inlineMachinery;
  for (const [key, id] of Object.entries(HOMEWORK_INPUTS)) byId<HTMLInputElement>(id).value = settings[key] ?? "";
  byId<HTMLInputElement>("homeworkProblemPrefixInput").value = settings.problemPrefix ?? "";
  byId<HTMLSelectElement>("homeworkProblemStyleSelect").value = settings.problemStyle || "arabic";
  byId<HTMLSelectElement>("homeworkSectionStyleSelect").value = settings.sectionStyle || "alph";
  byId<HTMLSelectElement>("homeworkSectionModeSelect").value = settings.sectionMode || "standalone";
  ...
  for (const id of [...Object.values(HOMEWORK_INPUTS), "homeworkProblemPrefixInput", "homeworkProblemStyleSelect", "homeworkSectionStyleSelect", "homeworkSectionModeSelect"]) {
    byId<HTMLInputElement>(id).disabled = !numbering;
  }
  validateHomeworkForm();
  byId<HTMLButtonElement>("saveHomeworkSettingsBtn").disabled = !numbering;
  renderHomeworkPreview(readHomeworkForm());
}

async function saveHomeworkSettings(): Promise<void> {
...
async function enableHomeworkHooks(): Promise<void> {
...
function renderSyncPanel(): void {
  ...
  itemList.querySelectorAll<HTMLButtonElement>("[data-sync-action]").forEach(button => button.addEventListener("click", () => run(async () => {
    const command = button.dataset.syncAction === "diff"
      ? "overleaf-open-diff"
      : button.dataset.syncAction === "push"
        ? "overleaf-push"
        : button.dataset.syncAction === "pull"
          ? "overleaf-pull"
          : button.dataset.syncAction === "retry"
            ? "overleaf-retry"
            : "overleaf-trash";
    await request(command, overleafPayload({ path: button.dataset.syncPath }));
    await refreshOverleafState();
  })));
  conflictList.querySelectorAll<HTMLButtonElement>("[data-conflict-action]").forEach(button => button.addEventListener("click", () => run(async () => {
...
async function applySelectedSync(): Promise<void> {
  ...
  const selected = new Set<string>(items.map((item: any) => String(item.path)));
  await request("overleaf-bulk-sync", overleafPayload({ paths: [...selected] }));
  await refreshOverleafState();
  for (const path of selected) {
    const current = (overleafState?.syncItems || []).find((item: any) => item.path === path);
...
/**
 * Generating never wrote theme.colors.tex, so a new project silently inherited the
 * defaults baked into theme.sty. Offering the theme here is only meaningful for starters
 * that load theme.sty at all: research-paper and the Beamer decks are self-contained,
 * which the existing toolkit-theme capability already records.
 */
function renderStarterStyle(): void {
  const select = byId<HTMLSelectElement>("starterStylePreset");
  const row = byId("starterStyleRow");
  const hint = byId("starterStyleHint");
  const selected = (model.schema.starter_templates || []).find((item: any) => item.id === starterTemplateSelection);
  ...
  renderSelect(select, presets.map((preset: any) => ({ value: preset.id, label: preset.label })), select.value || model.state.style_preset || "default");
}
...
function renderTargets(): void {
  const mode = byId<HTMLSelectElement>("compileModeSelect");
...
function renderBuildContext(): void {
  ...
  byId<HTMLSelectElement>("targetSelect").disabled = remote;
  byId<HTMLSelectElement>("recipeSelect").disabled = remote;
  byId<HTMLInputElement>("useInternalFallback").disabled = remote;
  ...
  if (noTargets) {
    ...
    description.textContent = compiledAt
      ? `Remote output from ${new Date(compiledAt).toLocaleString()}`
      : overleafState.mirrorRoot || "Open an Overleaf mirror before remote compile.";
...
async function refreshPdfStatus(): Promise<void> {
  ...
  renderBuildContext();
  try {
    const result = overleafState?.compileMode === "overleaf"
      ? await request("overleaf-pdf-status", {})
      : await request("pdf-status", { path: requestedPath });
...
async function bootstrapStarter(): Promise<void> {
  ...
  const result = await request("template-bootstrap", {
    ...
    overwrite,
    style_preset: themed ? byId<HTMLSelectElement>("starterStylePreset").value : undefined
  });
...
function wire(): void {
  ...
  byId("syncLoginBtn").addEventListener("click", () => run(async () => { await request("overleaf-login", overleafPayload()); await refreshOverleafState(); }));
  byId("syncProjectsBtn").addEventListener("click", () => run(async () => { await request("overleaf-list-projects", overleafPayload()); await refreshOverleafState(); }));
  byId("syncOpenBtn").addEventListener("click", () => run(async () => { await request("overleaf-open-project", overleafPayload()); await refreshOverleafState(); }));
  byId("syncStartBtn").addEventListener("click", () => run(async () => { await request("overleaf-start-sync", overleafPayload()); await refreshOverleafState(); }));
  ...
  // The preview is the only feedback for how these combine, so keep it live.
...
function isHomeworkWorkspace(): boolean {
  // A homework project is an ordinary article, so the kind cannot distinguish it.
...
function availableSections(): ToolkitSection[] {
  ...
  // Homework keeps every article section — it still uses theme.sty — and only adds one.
  const homework = isHomeworkWorkspace();
