...
function wire(): void {
  ...
  byId("syncLoginBtn").addEventListener("click", () => run(async () => { await request("overleaf-login", {}); await refreshOverleafState(); }));
  byId("syncStartBtn").addEventListener("click", () => run(async () => { await request("overleaf-start-sync", {}); await refreshOverleafState(); }));
  byId("syncStopBtn").addEventListener("click", () => run(async () => { await request("overleaf-stop-sync", {}); await refreshOverleafState(); }));
  byId("syncCheckBtn").addEventListener("click", () => run(async () => { await request("overleaf-check-sync", { mode: "incremental" }); await refreshOverleafState(); }));
