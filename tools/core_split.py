...
def renumber_compile_target(
    ...
    *,
    ...
    if mode_normalized not in {"add", "remove"}:
        raise ValueError("Unsupported renumber mode. Expected one of: add, remove")
    ...
    result = splitter.renumber_references(
        ctx.target_abs,
    ...
    for source, target in sorted(
        result.renamed.items(),
        key=lambda item: safe_workspace_relpath(item[0]),
    ):
        source_rel = safe_workspace_relpath(source)
        target_rel = safe_workspace_relpath(target)
        if source_rel and target_rel:
            renamed[source_rel] = target_rel

    warnings = list(result.warnings)
    if result.dry_run:
        if "Dry-run mode enabled; no files were written." not in warnings:
            warnings.append("Dry-run mode enabled; no files were written.")

    updated_files = [safe_workspace_relpath(path) for path in result.updated_files]
...
def unsplit_compile_target(
    ...
    *,
    ...
    ctx = resolve_compile_context(compile_target)
    result = splitter.unsplit_one_unit(
        ctx.target_abs,
        dry_run=bool(dry_run),
    ...
    updated_files = [safe_workspace_relpath(path) for path in result.updated_files]
    updated_files = [path for path in updated_files if path]
    root_target = safe_workspace_relpath(result.root_path)
    source_target = safe_workspace_relpath(result.unit_path)
    ...
    return {
        ...
        "backup_path": safe_workspace_relpath(result.backup_path),
        "dry_run": bool(result.dry_run),
        "deleted_source": bool(result.deleted_source),
        "updated_files": updated_files,
        "warnings": warnings,
        "suggested_compile_target": root_target,
    }
