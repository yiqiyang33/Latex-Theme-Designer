...
def parse_main_toggle_defaults(
    *,
    main_tex_path: Path,
...
def parse_toggle_override_file(
    path: Path,
    *,
    read_text_fn: Callable[[Path], str],
    toggle_schema: List[Dict[str, str]],
    ...
    for entry in toggle_schema:
        ...
        if matches:
            ...
            if parsed is not None:
                found[entry["id"]] = bool(parsed)
    return found


def parse_class_override_file(
    path: Path,
    *,
    read_text_fn: Callable[[Path], str],
    ...
    for field_id in class_config_ids:
        ...
        if matches:
            parsed[field_id] = normalize_class_config_value_fn(field_id, matches[-1])
    return parsed


def parse_body_font_size_override(
    path: Path,
    *,
    read_text_fn: Callable[[Path], str],
    ...
    if not matches:
        return None
    return normalize_body_font_size_value_fn(matches[-1])


def parse_color_override_file(
    path: Path,
    *,
    read_text_fn: Callable[[Path], str],
    color_set: set[str],
    ...
    for token, mapped in re.findall(r"\\colorlet\{([^}]+)\}\{([^}]+)\}", text):
        ...
        if mapped in define_map:
            ...
            continue
        parsed_hex = parse_hex_color_fn(mapped)
        if parsed_hex:
...
def coerce_class_mode_on_target_switch(
    state: Dict[str, Any],
    previous_target: str,
    next_target: str,
    *,
    normalize_class_config_map_fn: Callable[[Dict[str, Any]], Dict[str, str]],
    detect_target_documentclass_fn: Callable[[str], str],
    is_incompatible_forced_theme_class_fn: Callable[[str, str], bool],
) -> bool:
    ...
    detected = detect_target_documentclass_fn(nxt)
    if not detected:
        return False

    if is_incompatible_forced_theme_class_fn(mode, detected):
...
def write_override_files(
    ...
    *,
    ...
    toggle_lines = [
        "% Auto-generated by tools/latex_toolkit.py (legacy alias: tools/theme_designer.py)",
