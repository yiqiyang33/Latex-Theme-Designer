...
(() => {
  ...
  var syncSelectionMode = false;
  var selectedSyncPaths = /* @__PURE__ */ new Set();
  var publishForm = null;
  var publishLoading = false;
  var publishBusy = false;
  var publishExcluded = /* @__PURE__ */ new Set();
  ...
  function renderWorkspaceMode() {
    ...
    document.querySelectorAll("[data-section-target]").forEach((button) => {
      button.hidden = !sections.has(button.dataset.sectionTarget);
    });
  ...
  function renderBeamerSettings() {
    ...
    byId("beamerTemplateName").textContent = templateId === "beamer-generic" ? "Generic Beamer" : (model.schema.starter_templates || []).find((item) => item.id === templateId)?.label || templateId;
    ...
    byId("beamerResourceStatus").textContent = assetsComplete === true ? "Complete" : assetsComplete === false ? `Missing (${missingAssets.length})` : "Unknown";
    ...
    for (const id of ["beamerTitleInput", "beamerAuthorInput", "beamerInstituteInput", "beamerDateInput"]) {
      byId(id).disabled = !metadataEnabled;
    }
    byId("beamerAspectRatioSelect").disabled = !hooksEnabled || !capabilities.has("aspect-ratio");
  ...
  async function saveBeamerSettings() {
    const settings = {
      title: byId("beamerTitleInput").value,
      author: byId("beamerAuthorInput").value,
      institute: byId("beamerInstituteInput").value,
      date: byId("beamerDateInput").value,
      aspectRatio: byId("beamerAspectRatioSelect").value,
      notesMode: byId("beamerNotesModeSelect").value,
      sectionOutline: byId("beamerSectionOutlineInput").checked
    };
  ...
  async function enableBeamerToolkit() {
  ...
  function renderSyncPanel() {
    ...
    if (!state?.available) {
      ...
      byId("syncContextDescription").textContent = publishCardOpen() ? "Review what will be uploaded, then publish. The folder becomes a live mirror of the new project." : "Open an Overleaf mirror to inspect realtime status.";
    ...
    const selectablePaths = new Set(items.filter(isSelectableSyncItem).map((item) => item.path));
    ...
    byId("syncPreviewBtn").textContent = syncSelectionMode ? "Cancel selection" : "Preview and select";
    byId("syncApplySelectedBtn").hidden = !syncSelectionMode;
    byId("syncSelectionSummary").textContent = syncSelectionMode ? `${selectedSyncPaths.size} selected. Only safe remote-ahead/local-ahead changes can be applied in bulk.` : "";
    const conflicts = state.conflicts || [];
    const binaryConflicts = items.filter((item) => item.entityType === "file" && item.status === "diverged");
    ...
    const textConflictMarkup = conflicts.map((item) => `<div class="sync-item"><span><strong>${escapeHtml(item.relPath)}</strong><small>${escapeHtml(item.reason || "manual resolution required")}</small></span><span class="toolbar compact"><button class="icon-button" data-conflict-action="diff" data-conflict-path="${escapeHtml(item.relPath)}" aria-label="Open conflict diff for ${escapeHtml(item.relPath)}" title="Open three-way diff"><i class="codicon codicon-diff"></i></button><button class="icon-button" data-conflict-action="local" data-conflict-path="${escapeHtml(item.relPath)}" aria-label="Use local version for ${escapeHtml(item.relPath)}" title="Use local"><i class="codicon codicon-cloud-upload"></i></button><button class="icon-button" data-conflict-action="remote" data-conflict-path="${escapeHtml(item.relPath)}" aria-label="Accept remote version for ${escapeHtml(item.relPath)}" title="Accept remote"><i class="codicon codicon-cloud-download"></i></button></span></div>`).join("");
    const binaryConflictMarkup = binaryConflicts.map((item) => `<div class="sync-item"><span><strong>${escapeHtml(item.path)}</strong><small>Binary diverged \xB7 local ${formatBytes(item.localSize)}${item.localMtimeMs ? ` (${escapeHtml(new Date(item.localMtimeMs).toLocaleString())})` : ""} \xB7 remote ${formatBytes(item.remoteSize)} \xB7 local ${escapeHtml(shortHash(item.localHash))} \xB7 remote ${escapeHtml(shortHash(item.remoteHash))}</small></span><span class="toolbar compact"><button class="icon-button" data-sync-action="push" data-sync-path="${escapeHtml(item.path)}" aria-label="Keep local binary ${escapeHtml(item.path)}" title="Keep local"><i class="codicon codicon-cloud-upload"></i></button><button class="icon-button" data-sync-action="pull" data-sync-path="${escapeHtml(item.path)}" aria-label="Accept remote binary ${escapeHtml(item.path)}" title="Accept remote"><i class="codicon codicon-cloud-download"></i></button></span></div>`).join("");
    ...
    byId("syncActivityList").innerHTML = activity.length ? activity.slice(-20).reverse().map((entry) => `<li><time>${escapeHtml(new Date(entry.at).toLocaleTimeString())}</time><span>${escapeHtml(entry.message)}</span></li>`).join("") : "<li>No sync activity recorded.</li>";
    ...
    conflictList.querySelectorAll("[data-sync-action]").forEach((button) => button.addEventListener("click", () => run(async () => {
      await request(button.dataset.syncAction === "pull" ? "overleaf-pull" : "overleaf-push", overleafPayload({ path: button.dataset.syncPath }));
      await refreshOverleafState();
    })));
    itemList.querySelectorAll("[data-sync-select]").forEach((input) => input.addEventListener("change", () => {
      if (input.checked) selectedSyncPaths.add(input.dataset.syncSelect);
      else selectedSyncPaths.delete(input.dataset.syncSelect);
  ...
  function publishCardOpen() {
    return publishLoading || publishForm !== null;
  }
  async function openPublishForm(folder) {
  ...
  function showPublishForm(form) {
    publishLoading = false;
    publishForm = form;
    publishExcluded.clear();
    byId("publishNameInput").value = form.folderName || "";
    const rootSelect = byId("publishRootSelect");
    rootSelect.innerHTML = (form.roots || []).map((root) => `<option value="${escapeHtml(root)}">${escapeHtml(root)}</option>`).join("");
  ...
  function renderPublishFiles() {
    const form = publishForm;
    if (!form) return;
    const root = byId("publishRootSelect").value;
    publishExcluded.delete(root);
    byId("publishFileList").innerHTML = (form.files || []).map((file) => {
      const isRoot = file.path === root;
      const excluded = publishExcluded.has(file.path);
      const slash = file.path.lastIndexOf("/");
      return `<label class="publish-file" data-excluded="${excluded}" title="${escapeHtml(isRoot ? `${file.path} is the main document and is always uploaded` : file.path)}"><input type="checkbox" data-publish-path="${escapeHtml(file.path)}"${excluded ? "" : " checked"}${isRoot ? ' data-publish-root="true"' : ""}><span class="publish-file-name"><span class="publish-file-dir">${escapeHtml(file.path.slice(0, slash + 1))}</span>${escapeHtml(file.path.slice(slash + 1))}</span>` + (isRoot ? `<span class="context-badge">main</span>` : "") + `<span class="publish-file-size">${formatBytes(file.size)}</span></label>`;
    }).join("");
  ...
  function updatePublishSummary() {
    ...
    const included = files.filter((file) => !publishExcluded.has(file.path));
    const bytes = included.reduce((sum, file) => sum + (file.size || 0), 0);
    byId("publishFileSummary").textContent = `${included.length} of ${files.length} files \xB7 ${formatBytes(bytes)}`;
    const selectable = files.length - 1;
    const selectAll = byId("publishSelectAll");
  ...
  function renderPublishActions() {
    const form = publishForm;
    const nameInput = byId("publishNameInput");
    ...
    const submit = byId("publishSubmitBtn");
    ...
    for (const id of ["publishCancelBtn", "publishChangeFolderBtn", "publishLoginBtn", "publishRootSelect", "publishNameInput", "publishSelectAll"]) {
      byId(id).disabled = publishBusy;
    }
    byId("publishFileList").querySelectorAll("input[data-publish-path]").forEach((input) => {
  ...
  async function submitPublishForm() {
    ...
    try {
      await request("overleaf-publish", overleafPayload({
        folder: form.folder,
        projectName: byId("publishNameInput").value.trim(),
        rootDocPath: byId("publishRootSelect").value,
  ...
  async function changePublishFolder() {
    if (publishBusy) return;
  ...
  async function signInForPublish() {
  ...
  async function applySelectedSync() {
    const items = (overleafState?.syncItems || []).filter((item) => selectedSyncPaths.has(item.path) && isSelectableSyncItem(item));
    if (!items.length) {
  ...
  function syncItemMarkup(item) {
    ...
    return `<div class="sync-item">${selection}<span><strong>${encodedPath}</strong><small>${escapeHtml(item.status)}${item.message ? ` \xB7 ${escapeHtml(item.message)}` : ""}</small></span><span class="toolbar compact">${actions.join("")}</span></div>`;
  }
  ...
  function renderStarter() {
    const kindSelect = byId("starterKindSelect");
    ...
    const current = model.schema.starter_templates?.find((item) => item.id === starterTemplateSelection);
    const preferredKind = current?.kind || kindSelect.value || groups[0]?.id || "book";
    renderSelect(kindSelect, groups.map((group) => ({ value: group.id, label: group.label })), preferredKind);
    const templates = groups.find((group) => group.id === kindSelect.value)?.templates || model.schema.starter_templates || [];
  ...
  function renderStarterDescription() {
    ...
    const templates = (model.schema.starter_template_groups || []).find((group) => group.id === kind)?.templates || model.schema.starter_templates || [];
  ...
  function wire() {
    ...
    byId("syncOpenProjectBtn").addEventListener("click", () => run(async () => {
      await request("overleaf-open-project", overleafPayload());
      await refreshOverleafState();
    }));
    byId("publishChangeFolderBtn").addEventListener("click", () => void changePublishFolder().catch((err) => setPublishError(err.message)));
    byId("publishLoginBtn").addEventListener("click", () => void signInForPublish().catch((err) => setPublishError(err.message)));
    ...
    byId("publishSelectAll").addEventListener("change", () => {
      ...
      publishExcluded.clear();
      if (excludeAll) {
        for (const file of publishForm?.files || []) if (file.path !== root) publishExcluded.add(file.path);
      }
      renderPublishFiles();
    });
