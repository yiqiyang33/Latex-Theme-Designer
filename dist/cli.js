  ...
  "node_modules/ignore/index.js"(exports2, module2) {
    ...
    var REPLACERS = [
      ...
      [
        ...
        /(\\)?\[([^\]/]*?)(\\*)($|\])/g,
        (match2, leadEscape, range, endEscape, close) => leadEscape === ESCAPE ? `\\[${range}${cleanRangeBackSlash(endEscape)}${close}` : close === "]" ? endEscape.length % 2 === 0 ? `[${negateRange(sanitizeRange(range))}${endEscape}]` : "[]" : "[]"
      ],
      // ending
      [
    ...
    var Ignore2 = class {
      ...
      checkIgnore(path14) {
        ...
        const slices = path14.split(SLASH2).filter(Boolean);
        slices.pop();
        if (slices.length) {
      ...
      _t(path14, cache, checkUnignored, slices) {
        ...
        if (!slices) {
          slices = path14.split(SLASH2).filter(Boolean);
        }
        slices.pop();
        if (!slices.length) {
...
var require_mime_types = __commonJS({
  "node_modules/mime-types/index.js"(exports2) {
    ...
    function lookup2(path14) {
      if (!path14 || typeof path14 !== "string") {
        return false;
      }
      var extension2 = extname4("x." + path14).toLowerCase().substr(1);
      if (!extension2) {
        return false;
      }
...
var require_lib2 = __commonJS({
  "node_modules/node-fetch/lib/index.js"(exports2, module2) {
    ...
    function fetch2(url, opts) {
      ...
      return new fetch2.Promise(function(resolve6, reject) {
          ...
          if (fetch2.isRedirect(res.statusCode)) {
            ...
            switch (request.redirect) {
              ...
              case "follow":
                ...
                resolve6(fetch2(new Request(locationURL, requestOpts)));
                finalize();
                return;
            }
          }
...
var ReaddirpStream = class extends import_node_stream.Readable {
  ...
  async _exploreDir(path14, depth) {
    let files;
    try {
      files = await (0, import_promises.readdir)(path14, this._rdOptions);
    } catch (error) {
      this._onError(error);
    }
  ...
  async _formatEntry(dirent, path14) {
    ...
    try {
      const fullPath = (0, import_node_path.resolve)((0, import_node_path.join)(path14, basename5));
...
function createFsWatchInstance(path14, options, listener, errHandler, emitRaw) {
  ...
  try {
    return (0, import_fs.watch)(path14, {
  ...
  if (cont) {
    ...
    watcher.on(EV.ERROR, async (error) => {
      ...
      if (isWindows && error.code === "EPERM") {
        try {
          const fd = await (0, import_promises2.open)(path14, "r");
          await fd.close();
          broadcastErr(error);
        } catch (err) {
        }
...
var NodeFsHandler = class {
  ...
  _watchWithNodeFs(path14, listener) {
    ...
    const basename5 = sysPath.basename(path14);
  ...
  async _handleSymlink(entry, directory, path14, item) {
    ...
    if (!this.fsw.options.followSymlinks) {
      ...
      let linkPath;
      try {
        linkPath = await (0, import_promises2.realpath)(path14);
      } catch (e) {
        this.fsw._emitReady();
        return true;
      }
  ...
  async _addToNodeFs(path14, initialAdd, priorWh, depth, target) {
    ...
    try {
      ...
      if (stats.isDirectory()) {
        ...
        const targetPath = follow ? await (0, import_promises2.realpath)(path14) : path14;
        if (this.fsw.closed)
          return;
...
var FSWatcher = class extends import_events.EventEmitter {
  ...
  add(paths_, _origAdd, _internal) {
    ...
    Promise.all(paths.map(async (path14) => {
      const res = await this._nodeFsHandler._addToNodeFs(path14, !_internal, void 0, 0, _origAdd);
      if (res)
        this._emitReady();
      return res;
    })).then((results) => {
  ...
  _isntIgnored(path14, stat9) {
    return !this._isIgnored(path14, stat9);
  }
  /**
...
function formatUnknownError(error) {
  ...
  const main2 = message ?? nestedMessage;
  ...
  if (main2) {
    return details ? `${main2} (${details})` : main2;
  }
...
// src/overleaf/manifest.ts
var METADATA_DIR = ".overleaf-codex";
var MANIFEST_NAME = "manifest.json";
var OUTPUT_DIR = "output";
var BASE_DIR = "base";
var SYNC_STATUS_NAME = "sync-status.json";
...
var LEGACY_CONFIGURABLE_IGNORE_PATTERNS = [
  "**/*.aux",
  "**/*.bbl",
  "**/*.bcf",
  "**/*.bcf-*",
...
var DEFAULT_LOCAL_IGNORE_CONTENT = `# Overleaf Codex local-only paths.
...
function metadataPath(root, ...parts) {
  return path3.join(root, METADATA_DIR, ...parts);
}
...
async function ensureLocalIgnoreFile(root) {
  ...
  const existing = await fs.readFile(target, "utf8").catch(() => void 0);
  if (existing !== void 0) {
    return existing;
  }
  await fs.writeFile(target, DEFAULT_LOCAL_IGNORE_CONTENT, { encoding: "utf8", flag: "wx" }).catch(() => void 0);
...
function baseDocPath(root, docId) {
  return metadataPath(root, BASE_DIR, "docs", `${docId}.tex`);
}
async function readBaseDoc(root, docId) {
  return fs.readFile(baseDocPath(root, docId), "utf8").catch(() => void 0);
}
...
function syncStatusPath(root) {
  return metadataPath(root, SYNC_STATUS_NAME);
}
async function readSyncStatus(root) {
  const raw = await fs.readFile(syncStatusPath(root), "utf8").catch(() => void 0);
...
async function enqueueMetadataWrite(target, write) {
  ...
  const current = previous.catch(() => void 0).then(write);
...
function shouldIgnoreUntrackedLocalPath(manifest, relPath) {
  ...
  const matcher = localIgnoreRules.get(manifest) ?? (0, import_ignore.default)().add(DEFAULT_LOCAL_IGNORE_CONTENT);
  return matcher.ignores(normalized) || matcher.ignores(`${normalized.replace(/\/+$/, "")}/`);
}
function addOrUpdateFile(manifest, file, content) {
...
function addProjectTreeEntity(project, parentFolderId, kind, entity) {
  const parent = findProjectFolder(project, parentFolderId);
  if (!parent || findProjectEntity(project, entity._id)) return false;
  if (kind === "doc") (parent.docs ??= []).push(entity);
  else if (kind === "file") (parent.fileRefs ??= []).push(entity);
  else (parent.folders ??= []).push(entity);
  return true;
}
function renameProjectTreeEntity(project, entityId, newName) {
  const found = findProjectEntity(project, entityId);
  if (!found) return false;
  found.entity.name = newName;
  return true;
}
function updateProjectTreeDocVersion(project, docId, version) {
  const found = findProjectEntity(project, docId);
  if (!found || found.kind !== "doc") return false;
  found.entity.version = version;
  return true;
}
function moveProjectTreeEntity(project, entityId, newParentFolderId) {
  const target = findProjectFolder(project, newParentFolderId);
  const found = findProjectEntity(project, entityId);
  if (!target || !found || found.kind === "folder" && containsFolder(found.entity, newParentFolderId)) {
    return false;
  }
  found.collection.splice(found.index, 1);
  if (found.kind === "doc") (target.docs ??= []).push(found.entity);
  else if (found.kind === "file") (target.fileRefs ??= []).push(found.entity);
  else (target.folders ??= []).push(found.entity);
  return true;
}
function removeProjectTreeEntity(project, entityId) {
  const found = findProjectEntity(project, entityId);
  if (!found) return false;
  found.collection.splice(found.index, 1);
  return true;
...
function findFolderBelow(folder, folderId) {
...
function containsFolder(folder, folderId) {
  return folder._id === folderId || Boolean(findFolderBelow(folder, folderId));
}
function findProjectEntity(project, entityId) {
  return findEntityBelow(getRootFolder(project), entityId);
}
function findEntityBelow(folder, entityId) {
  const groups = [
...
function buildProjectTreeIndex(serverUrl, projectId, projectName, project) {
  const folders = [];
  const files = [];
  const root = getRootFolder(project);
  const manifest = {
...
function walkFolder(folder, folderPath, parentFolderId, folders, files) {
  ...
  for (const child of folder.folders ?? []) {
    const childPath = path4.posix.join(folderPath, child.name);
    walkFolder(child, childPath, folder._id, folders, files);
  }
  for (const doc of folder.docs ?? []) {
    files.push({
      path: path4.posix.join(folderPath, doc.name),
      entityId: doc._id,
      entityType: "doc",
  ...
  for (const file of folder.fileRefs ?? []) {
    files.push({
      path: path4.posix.join(folderPath, file.name),
      entityId: file._id,
      entityType: "file",
      parentFolderId: folder._id,
      binary: true,
...
function classifySyncStatus(input) {
  ...
  if (input.remoteReadError) {
    ...
    const localChanged = input.localHash !== void 0 && input.localHash !== baseHash;
    const remoteChanged = input.remoteHash !== void 0 && input.remoteHash !== baseHash;
  ...
  return {
    ...
    localPath: input.localExists ? input.path : void 0,
    remotePath: input.remoteFile ? input.path : void 0,
    changeKind: status === "error" ? "read-error" : status === "local only" || status === "remote only" ? "create" : status === "local deleted" || status === "remote deleted" ? "delete" : status === "synced" ? void 0 : "content"
  };
}
...
function classifyFolderStructure(manifest, remote, requestedPaths) {
  ...
  const requested = requestedPaths ? [...requestedPaths].map(toPosixPath) : void 0;
...
function repairFolderManifestFromRemote(manifest, remote, localFolderPaths) {
  const local = new Set([...localFolderPaths].map(toPosixPath));
  const adopted = [];
  const remapped = [];
  const remoteFolders = Object.values(remote.folders).filter((folder) => folder.path).sort((a, b) => a.path.split("/").length - b.path.split("/").length);
  for (const remoteFolder of remoteFolders) {
    if (manifest.folders[remoteFolder.path]) continue;
    const oldFolder = Object.values(manifest.folders).find((folder) => folder.entityId === remoteFolder.entityId);
...
function remapManifestSubtree(manifest, oldPath, newPath) {
  ...
  for (const folder of Object.values(manifest.folders)) {
    ...
    folder.path = folder.path === oldPath ? newPath : `${newPath}/${folder.path.slice(oldPrefix.length)}`;
    manifest.folders[folder.path] = folder;
  }
...
function makeSyncStatusReport(manifest, items, options = {}) {
...
function mergeTargetedSyncStatusReport(previous, targeted, requestedPaths) {
...
async function listLocalProjectFolders(root, manifest) {
  ...
  async function walk(absDir, relDir) {
    ...
    for (const entry of entries) {
      ...
      await walk(path5.join(root, relPath), relPath);
    }
  }
}
...
async function fileHash(filePath) {
  const content = await fs2.readFile(filePath).catch(() => void 0);
...
async function cachedLocalFileHash(filePath, manifestFile, force = false) {
  ...
  if (!stat9?.isFile()) {
    if (manifestFile) {
      ...
      return { hash: void 0, cacheChanged: cacheChanged2, reused: false };
    }
...
function buildOtOperations(before, after) {
  ...
  for (const [kind, text] of diffs) {
    if (kind === import_diff_match_patch.DIFF_EQUAL) position += text.length;
    else if (kind === import_diff_match_patch.DIFF_DELETE) ops.push({ p: position, d: text });
    else if (kind === import_diff_match_patch.DIFF_INSERT) {
...
var ConflictStore = class {
  ...
  async list() {
    const raw = await fs3.readFile(metadataPath(this.root, CONFLICT_INDEX_NAME), "utf8").catch(() => void 0);
  ...
  upsert(conflict) {
    return this.run(async () => {
      ...
      if (index >= 0) records[index] = conflict;
      else records.push(conflict);
  ...
  remap(oldPath, newPath, subtree = false) {
    return this.run(async () => {
      const oldPrefix = `${oldPath}/`;
...
`);
  ...
  run(operation) {
    const current = this.queue.catch(() => void 0).then(operation);
...
var BinaryTransactionStore = class {
  ...
  queue = Promise.resolve();
  async list() {
    const raw = await fs4.readFile(metadataPath(this.root, TRANSACTIONS_NAME), "utf8").catch(() => void 0);
    return raw ? JSON.parse(raw) : [];
  }
...
// src/overleaf/syncCommandCore.ts
function planSafeSyncActions(report, policy) {
  return {
    pulls: report.items.filter((item) => item.status === "remote ahead" || item.status === "remote only"),
    pushes: policy.autoPushLocalAhead ? report.items.filter(
      (item) => item.entityType !== "folder" && (item.status === "local ahead" || item.status === "local only") && (isTextLike(item.path) || policy.syncBinaryFiles)
    ) : []
  };
}
function selectRemoteWriteTarget(relPath, manifestFile, remoteFile, force) {
...
function syncOperationRequiresForce(operation, status) {
  if (!status) return false;
  return operation === "push" ? ["remote ahead", "remote deleted", "diverged", "local deleted"].includes(status) : ["local ahead", "local only", "remote deleted", "diverged"].includes(status);
}
async function executeSyncCommand(backend, command, args = {}) {
  switch (command) {
    case "status":
    ...
    case "push":
    case "pull": {
      const path14 = requiredPath(args.path);
      const force = Boolean(args.force);
      await backend.authorize?.(command, path14, force);
      await backend[command](path14, force);
      return backend.status({
        refresh: true,
        full: false,
        paths: [path14],
    ...
    case "conflicts-resolve": {
      const path14 = requiredPath(args.path);
      await backend.resolveConflict(path14, args.use === "remote" ? "remote" : "local");
      return backend.conflicts();
    }
    default:
...
async function performRemotePathChange(client, projectId, change, beforeMutation = () => void 0) {
  ...
  try {
    ...
    beforeMutation(change.entityId);
    await client.renameEntity(projectId, change.entityType, change.entityId, change.oldName).catch(() => void 0);
    throw error;
  }
  try {
    beforeMutation(change.entityId);
    ...
    beforeMutation(change.entityId);
    await client.moveEntity(projectId, change.entityType, change.entityId, change.oldParentFolderId).catch(() => void 0);
    beforeMutation(change.entityId);
    await client.renameEntity(projectId, change.entityType, change.entityId, change.oldName).catch(() => void 0);
    throw error;
  }
}
async function recoverBinaryTransactions(client, projectId, manifest, store, options = {}) {
  ...
  for (const transaction of records) {
    ...
    try {
      if (transaction.stage === "temp-uploaded") {
        await client.deleteEntity(projectId, "file", transaction.tempEntityId).catch(() => void 0);
        ...
        await client.deleteEntity(projectId, "file", transaction.originalEntityId).catch(() => void 0);
...
function transactionName(filename, suffix) {
  const ext2 = path6.posix.extname(filename);
  const stem = path6.posix.basename(filename, ext2);
...
var OverleafSyncEngine = class {
  constructor(root, client, policy, host) {
    this.root = root;
    this.client = client;
    this.policy = policy;
    this.host = host;
  ...
  events = new import_events2.EventEmitter();
  async start() {
  ...
  async status(refresh = false, full = false, paths, reason = "status") {
  ...
  async syncOnce() {
    ...
    for (const item of plan.pulls) {
      if (item.entityType === "folder") {
        await fs5.mkdir(path7.join(this.root, item.path), { recursive: true });
      } else {
  ...
  async watch() {
    await this.start();
    await this.syncOnce();
    this.watcher = esm_default.watch(this.root, {
      ignoreInitial: true,
    ...
    for (const event of ["add", "change", "unlink", "addDir", "unlinkDir"]) {
      this.watcher.on(event, (changed) => {
        this.host.log(`Local ${event}: ${path7.relative(this.root, changed)}`);
        this.scheduleSync(`local:${event}`);
      });
    }
    await new Promise((resolve6) => this.events.once("stop", resolve6));
  }
  requestStop() {
  ...
  async check(mode, options = {}) {
    return this.serial(async () => {
      ...
      for (const relPath of [...paths].sort()) {
        ...
        const localResult = await cachedLocalFileHash(path7.join(this.root, relPath), manifestFile, mode === "full");
        let remoteContent;
        let remoteReadError;
        if (remoteFile) {
          try {
            remoteContent = remoteFile.entityType === "doc" ? (await this.session.joinDoc(remoteFile.entityId)).content : await this.client.downloadProjectFile(this.manifest.projectId, remoteFile.entityId);
          } catch (error) {
            remoteReadError = formatUnknownError(error);
          }
        }
        const remoteHash = remoteContent === void 0 ? void 0 : sha1(remoteContent);
        ...
        if (!baseHash && remoteFile?.entityType === "doc") {
          ...
          baseHash = base === void 0 ? void 0 : sha1(base);
        }
        const item = classifySyncStatus({
          ...
          remoteHash,
          baseHash,
          localExists: localResult.hash !== void 0,
          remoteReadError
        });
        if (item.status === "diverged" && remoteFile && remoteContent !== void 0 && !existingConflicts.some((conflict) => conflict.relPath === relPath)) {
          const suffix = path7.extname(relPath) || (remoteFile.entityType === "doc" ? ".tex" : ".remote");
          const conflictPath = metadataPath(
            this.root,
            "conflicts",
            `${relPath.replace(/[\/\\]/g, "__")}.remote.${Date.now()}${suffix}`
          );
          await fs5.mkdir(path7.dirname(conflictPath), { recursive: true });
          await fs5.writeFile(conflictPath, remoteContent);
        ...
        if (!manifestFile && remoteFile && localResult.hash === remoteHash && remoteHash !== void 0) {
          addOrUpdateFile(this.manifest, remoteFile, remoteContent);
          this.manifest.files[relPath].baseHash = remoteFile.entityType === "doc" ? remoteHash : void 0;
        }
  ...
  async push(relPath, force) {
    await this.serial(async () => {
      ...
      const content = await fs5.readFile(path7.join(this.root, normalized)).catch(() => void 0);
      ...
      if (isTextLike(normalized)) {
        ...
        if (!target) {
          const doc = await this.client.addDoc(this.manifest.projectId, parentFolderId, path7.posix.basename(normalized));
        ...
        const uploaded = effectiveEntry ? await this.replaceBinary(normalized, content, effectiveEntry) : await this.client.uploadFile(this.manifest.projectId, parentFolderId, path7.posix.basename(normalized), content);
  ...
  async pull(relPath, force) {
    await this.serial(async () => {
      ...
      if (!remote) {
        ...
        const source = path7.join(this.root, normalized);
        const target = trashPathFor(this.root, normalized);
        const stat9 = await fs5.stat(source).catch(() => void 0);
        if (stat9) {
          await fs5.mkdir(path7.dirname(target), { recursive: true });
      ...
      const local = await fs5.readFile(path7.join(this.root, normalized)).catch(() => void 0);
      ...
      await fs5.mkdir(path7.dirname(path7.join(this.root, normalized)), { recursive: true });
      await fs5.writeFile(path7.join(this.root, normalized), remote.content);
  ...
  async resolveConflict(relPath, use) {
    const normalized = this.validatePath(relPath);
    ...
    await fs5.rm(conflict.remotePath, { force: true });
    await store.remove(normalized);
    this.emit("conflict-resolved", { path: normalized, use });
  }
  async remoteFile(relPath) {
    const project = this.session.getProject();
    if (!project || !this.manifest) return void 0;
  ...
  async ensureRemoteParentFolders(relPath) {
    const parent = path7.posix.dirname(relPath);
  ...
  async replaceBinary(relPath, content, entry) {
    try {
      return await this.client.uploadFile(
        this.manifest.projectId,
        entry.parentFolderId,
        path7.posix.basename(relPath),
        content
      );
    } catch (error) {
    ...
    const finalName = path7.posix.basename(relPath);
    ...
    const verified = temporary.hash === expected || gitBlobHash(await this.client.downloadProjectFile(this.manifest.projectId, temporary._id)) === expected;
    if (!verified) {
      await this.client.deleteEntity(this.manifest.projectId, "file", temporary._id).catch(() => void 0);
    ...
    await store.upsert(transaction);
    await this.client.deleteEntity(this.manifest.projectId, "file", entry.entityId).catch(() => void 0);
  ...
  async reconcileRemoteRenames(remote, localFiles, localFolders) {
    ...
    for (const oldEntry of Object.values(this.manifest.files)) {
      ...
      const hash = await cachedLocalFileHash(path7.join(this.root, oldEntry.path), oldEntry);
      if (hash.hash !== oldEntry.sha1) continue;
      await fs5.mkdir(path7.dirname(path7.join(this.root, newEntry.path)), { recursive: true });
      await fs5.rename(path7.join(this.root, oldEntry.path), path7.join(this.root, newEntry.path));
    ...
    for (const oldFolder of Object.values(this.manifest.folders)) {
      ...
      await fs5.mkdir(path7.dirname(path7.join(this.root, newFolder.path)), { recursive: true });
      await fs5.rename(path7.join(this.root, oldFolder.path), path7.join(this.root, newFolder.path));
    }
  }
  async reconcileLocalRenames(localFiles, localFolders) {
    ...
    const localCandidates = await Promise.all(untrackedFiles.map(async (candidate) => ({
      path: candidate,
      hash: await fs5.readFile(path7.join(this.root, candidate)).then(sha1)
    })));
    for (const oldFile of missingFiles) {
      const expected = oldFile.localHashCache ?? oldFile.sha1;
      if (!expected) continue;
      const matches = localCandidates.filter((candidate) => candidate.hash === expected && (isTextLike(candidate.path) ? "doc" : "file") === oldFile.entityType);
      const oldMatches = missingFiles.filter((candidate) => (candidate.localHashCache ?? candidate.sha1) === expected && candidate.entityType === oldFile.entityType);
      if (matches.length !== 1 || oldMatches.length !== 1) {
  ...
  async applyLocalRename(oldPath, newPath) {
    ...
    const oldName = path7.posix.basename(oldPath);
    const newName = path7.posix.basename(newPath);
  ...
  remapManifestFolder(oldPath, newPath, parentFolderId) {
  ...
  async folderFingerprintFromLocal(relPath) {
    const parts = [];
    const walk = async (absolute, relative4) => {
      for (const entry of await fs5.readdir(absolute, { withFileTypes: true }).catch(() => [])) {
        const child = toPosixPath(path7.posix.join(relative4, entry.name));
        const projectPath = toPosixPath(path7.posix.join(relPath, child));
        ...
        if (entry.isDirectory()) {
          ...
          await walk(path7.join(absolute, entry.name), child);
        } else if (entry.isFile()) {
          const content = await fs5.readFile(path7.join(absolute, entry.name));
          parts.push(`F\0${child}\0${isTextLike(child) ? "doc" : "file"}\0${sha1(content)}`);
        }
      }
    };
    await walk(path7.join(this.root, relPath), "");
    return sha1(`folder\0${parts.sort().join("\n")}`);
  }
  scheduleSync(reason) {
    if (!this.running) return;
  ...
  ignoreAbsolutePath(candidate) {
    const rel = toPosixPath(path7.relative(this.root, candidate));
  ...
  validatePath(relPath) {
    const normalized = toPosixPath(relPath);
    if (!normalized || normalized.startsWith("..") || path7.isAbsolute(relPath)) throw new Error(`Invalid project path: ${relPath}`);
    return normalized;
  }
  emit(event, data) {
  ...
  serial(operation) {
    const current = this.operation.catch(() => void 0).then(operation);
    this.operation = current.then(() => void 0, () => void 0);
    return current;
  }
};
...
function applicationSupportRoot() {
  return process.env.LATEX_TOOLKIT_SUPPORT_HOME ? path8.resolve(process.env.LATEX_TOOLKIT_SUPPORT_HOME) : path8.join(os.homedir(), "Library", "Application Support", "latex-editing-toolkit");
}
function runtimeRoot() {
  return process.env.LATEX_TOOLKIT_CACHE_HOME ? path8.resolve(process.env.LATEX_TOOLKIT_CACHE_HOME) : path8.join(os.homedir(), "Library", "Caches", "latex-editing-toolkit", "runtime");
}
function sharedStatePath() {
  return path8.join(applicationSupportRoot(), "overleaf.json");
}
function defaultSharedState() {
  return {
    ...
    localProjectsRoot: path8.join(os.homedir(), "Documents", "OverleafCodex", "projects")
  };
}
async function readSharedState() {
  const raw = await fs6.readFile(sharedStatePath(), "utf8").catch(() => void 0);
  ...
  return {
    ...
    credentialMigrations: Array.isArray(parsed.credentialMigrations) ? [...new Set(parsed.credentialMigrations.map(normalizeServerUrl))].sort() : [],
    credentialTombstones: Array.isArray(parsed.credentialTombstones) ? [...new Set(parsed.credentialTombstones.map(normalizeServerUrl))].sort() : [],
    policy: {
      ...defaults2.policy,
      ...parsed.policy ?? {},
      networkTimeouts: {
        ...defaults2.policy.networkTimeouts,
...
async function registerSharedMirror(root) {
  ...
  const record = {
    root: absolute,
    name: manifest.projectName || path8.basename(absolute),
    projectId: manifest.projectId,
    serverUrl: normalizeServerUrl(manifest.serverUrl),
    lastSyncAt: manifest.lastSyncAt
  };
  await updateSharedState((state) => {
    state.mirrors = [record, ...state.mirrors.filter((item) => path8.resolve(item.root) !== absolute)];
  });
  return record;
}
async function listSharedMirrors() {
  const state = await readSharedState();
  const records = [];
...
function dedupeMirrors(records) {
  ...
  for (const record of records.filter(isMirrorRecord)) result.set(path8.resolve(record.root), record);
...
async function exists(target) {
  return fs6.stat(target).then(() => true, () => false);
}

// src/overleaf/keychainStore.ts
var execFileAsync = (0, import_util9.promisify)(import_child_process.execFile);
...
var MacKeychainCredentialStore = class {
  ...
  async getIdentity(serverUrl) {
    ...
    try {
      ...
      if (isMissingKeychainItem(error)) return void 0;
      throw error;
    }
  }
...
async function compileRemoteProject(root, client, rootDocOverride) {
  ...
  await fs7.rm(outputRoot, { recursive: true, force: true });
  await fs7.mkdir(outputRoot, { recursive: true });
  const files = [];
  for (const output of response.outputFiles ?? []) {
    if (!output.url) continue;
    const target = path9.join(outputRoot, compileOutputName(output));
    await fs7.mkdir(path9.dirname(target), { recursive: true });
    await fs7.writeFile(target, await client.downloadCompileOutput(output.url, response));
    files.push(target);
  }
...
async function latestRemotePdf(root) {
  const outputRoot = metadataPath(root, OUTPUT_DIR);
  const entries = await fs7.readdir(outputRoot, { withFileTypes: true }).catch(() => []);
  return entries.find((entry) => entry.isFile() && entry.name.toLowerCase().endsWith(".pdf")) ? path9.join(outputRoot, entries.find((entry) => entry.isFile() && entry.name.toLowerCase().endsWith(".pdf")).name) : void 0;
}
async function detectRootDoc(root) {
  const manifest = await readManifest(root);
  for (const file of Object.values(manifest.files)) {
    if (!file.path.endsWith(".tex")) continue;
    const content = await fs7.readFile(path9.join(root, file.path), "utf8").catch(() => "");
...
var execFileAsync2 = (0, import_util11.promisify)(import_child_process2.execFile);
async function createProjectMirror(client, project, parentRoot) {
  const targetRoot = path10.join(parentRoot, sanitizeProjectFolderName(project.name, project.id));
  if (await exists2(targetRoot)) throw new Error(`Mirror target already exists: ${targetRoot}`);
  ...
  try {
    ...
    await fs8.mkdir(targetRoot, { recursive: true });
    for (const folder of index.folders) if (folder.path) await fs8.mkdir(path10.join(targetRoot, folder.path), { recursive: true });
    for (const file of index.files) await writeInitialFile(client, session, project.id, targetRoot, file);
    for (const name of ["output", "conflicts", path10.join("base", "docs"), "trash"]) {
      await fs8.mkdir(metadataPath(targetRoot, name), { recursive: true });
    }
    ...
    await fs8.rm(targetRoot, { recursive: true, force: true });
    throw error;
  } finally {
    session.disconnect();
  }
...
async function writeInitialFile(client, session, projectId, root, file) {
  const target = path10.join(root, file.path);
  ...
  if (file.entityType === "doc") {
    ...
    await fs8.writeFile(target, joined.content, "utf8");
    ...
    await fs8.writeFile(target, content);
    file.binary = true;
    file.sha1 = sha1(content);
  }
}
async function writeMirrorSupportFiles(root, rootDocPath) {
...
`;
  ...
  await fs8.mkdir(path10.join(root, ".vscode"), { recursive: true });
  await Promise.all([
    fs8.writeFile(path10.join(root, "AGENTS.md"), agents, "utf8"),
    fs8.writeFile(path10.join(root, ".gitignore"), gitignore, "utf8"),
    fs8.writeFile(path10.join(root, ".vscode", "settings.json"), `${JSON.stringify(settings, null, 2)}
`, "utf8"),
    fs8.writeFile(path10.join(root, ".latexmkrc"), `$out_dir = '.overleaf-codex/local-build';
$aux_dir = $out_dir;
`, "utf8")
  ]);
}
async function initializeGit(root, message) {
  await execFileAsync2("git", ["init"], { cwd: root, encoding: "utf8" });
  await execFileAsync2("git", ["add", "-A"], { cwd: root, encoding: "utf8" });
  await execFileAsync2("git", [
    "-c",
    "user.name=Overleaf Codex",
    "-c",
    "user.email=overleaf-codex@local",
    "commit",
    "-m",
    message
  ], { cwd: root, encoding: "utf8" }).catch(() => void 0);
}
...
var OverleafHttpError = class extends Error {
...
var OverleafClient = class {
  constructor(serverUrl, identity, timeoutSeconds, timeouts = {}) {
    ...
    const legacyMs = (timeoutSeconds ?? 60) * 1e3;
    this.timeouts = {
      ...DEFAULT_TIMEOUTS,
      httpMs: legacyMs,
      ...timeouts
    ...
    this.agent = new URL(this.serverUrl).protocol === "http:" ? new http.Agent({ keepAlive: true }) : new https.Agent({ keepAlive: true });
  }
  serverUrl;
  agent;
  identity;
  ...
  async listProjects() {
    try {
      const result = await this.requestJson("POST", "api/project", {
        body: {}
      });
  ...
  async addFolder(projectId, parentFolderId, folderName) {
    return this.requestJson("POST", `project/${projectId}/folder`, {
      body: {
        parent_folder_id: parentFolderId,
        name: folderName
      },
  ...
  async uploadFile(projectId, parentFolderId, filename, content) {
    const form = new import_form_data.default();
    form.append("targetFolderId", parentFolderId);
    form.append("name", filename);
    form.append("type", mime.lookup(filename) || "application/octet-stream");
    form.append("qqfile", import_stream.Readable.from(content), { filename });
    const result = await this.requestJson("POST", `project/${projectId}/upload?folder_id=${encodeURIComponent(parentFolderId)}`, {
      body: form,
      includeCsrfHeader: true
    });
    if (!result.entity_id) {
  ...
  async connectSocket(projectId, signal) {
    ...
    try {
      ...
      first.disconnect();
      const fallback = new OverleafSocketSession(this.serverUrl, this.identity, this.timeouts, void 0);
      try {
  ...
  async fetchWithTimeout(url, init = {}, timeoutMs = this.timeouts.httpMs, externalSignal) {
  ...
  async requestJson(method, route, options = {}) {
    const text = await this.requestText(method, route, options);
    return text ? JSON.parse(text) : {};
  }
  async requestText(method, route, options = {}) {
    ...
    let body;
    if (options.body instanceof import_form_data.default) {
    ...
    while (true) {
      ...
      if (res.status >= 300 && res.status < 400) {
        ...
        if (!location || redirects >= 5) {
          throw new OverleafHttpError(`Overleaf download redirect failed for ${path11.basename(currentUrl)}.`, res.status);
        }
...
var OverleafSocketSession = class {
  constructor(serverUrl, identity, timeouts, query) {
    ...
    const cliRuntimeRoot = path11.join(__dirname, "cli-vendor", "socket.io-client");
    const runtimeRoot2 = (0, import_fs3.existsSync)(path11.join(cliRuntimeRoot, "lib", "io.js")) ? cliRuntimeRoot : path11.join(__dirname, "vendor", "socket.io-client");
  ...
  async joinProject(projectId, signal) {
    const rejectPromise = new Promise((_, reject) => {
      this.socket.once("connectionRejected", (error) => {
        reject(new Error(error?.message || "Overleaf rejected the realtime connection."));
      });
    });
    const joinPromise = this.emitAck("joinProject", this.timeouts.projectJoinMs, signal, { project_id: projectId }).then((values) => values[0]);
  ...
  waitForJoinProjectResponse(signal, timeoutMs) {
    ...
    return new Promise((resolve6, reject) => {
      ...
      const finish = (error, project) => {
        ...
        settled = true;
        cleanup();
        error ? reject(error) : resolve6(project);
      };
  ...
  async leaveDoc(docId) {
    await this.emitAck("leaveDoc", this.timeouts.joinDocMs, void 0, docId);
  }
  async applyOtUpdate(docId, update, signal) {
  ...
  async updatePosition(docId, row, column) {
    await this.emitAck("clientTracking.updatePosition", this.timeouts.otAckMs, void 0, {
      doc_id: docId,
      row,
      column
    });
  ...
  emitAck(event, timeoutMs, signal, ...args) {
    return new Promise((resolve6, reject) => {
      ...
      const finish = (error, values) => {
        ...
        settled = true;
        cleanup();
        error ? reject(error) : resolve6(values ?? []);
      };
      const onAbort = () => finish(abortError(signal));
...
function parseContentRange(value) {
  ...
  return { start: Number(match2[1]), end: Number(match2[2]), total: Number(match2[3]) };
}
function abortError(signal) {
  return signal?.reason instanceof Error ? signal.reason : new Error("Operation cancelled.");
}
function loadSocketIoClient(runtimeRoot2 = path11.join(__dirname, "vendor", "socket.io-client")) {
  const requireFromExtension = (0, import_module.createRequire)(__filename);
  const entry = path11.join(runtimeRoot2, "lib", "io.js");
  let loaded;
  try {
...
function loadSocketIoWebSocket(runtimeRoot2) {
  ...
  try {
    ...
    const message = formatUnknownError(error);
    throw new Error(`Could not load the Overleaf WebSocket runtime from ${path11.join(runtimeRoot2, "node_modules", "ws")}: ${message}. Rebuild or reinstall the extension.`);
  }
...
function patchSocketIoHandshake(socketIo, runtimeRoot2) {
  ...
  const Ws = originalWebsocketOpen ? loadSocketIoWebSocket(runtimeRoot2) : void 0;
  if (originalWebsocketOpen) {
    socketIo.Transport.websocket.prototype.open = function patchedWebsocketOpen() {
      ...
      this.websocket.onmessage = (event) => this.onData(event.data);
  ...
  if (originalXhrRequest) {
    socketIo.Transport.XHR.prototype.request = function patchedXhrRequest(method) {
  ...
  socketIo.Socket.prototype.handshake = function patchedHandshake(fn) {
    ...
    requestSocketHandshake(url, {
      ...
      timeoutMs: this.options.overleafCodexTimeoutMs ?? 6e4
    }).then((result) => {
...
async function requestSocketHandshake(url, options) {
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    ...
    try {
      ...
      await new Promise((resolve6) => setTimeout(resolve6, 250 * 2 ** attempt));
    }
  }
...
function parseSocketHandshakeBody(body) {
  ...
  const match2 = /^([^:\s]+):(\d+):(\d+):([a-z0-9_-]+(?:,[a-z0-9_-]+)*)$/i.exec(normalized);
  if (!match2) {
...
function requestSocketHandshakeOnce(url, options) {
  return new Promise((resolve6, reject) => {
    ...
    request.setTimeout(options.timeoutMs, () => {
      request.destroy(new Error(`Timed out fetching the Overleaf socket handshake after ${options.timeoutMs / 1e3} seconds.`));
    });
...
function extractFirst(input, patterns) {
  for (const pattern of patterns) {
    const match2 = pattern.exec(input);
    if (match2?.[1]) {
      return match2[1];
    }
  }
...
async function assertOk(res, route) {
  ...
  try {
    ...
    code = typeof parsed.error === "string" ? parsed.error : void 0;
  } catch {
    code = void 0;
  }
  throw new OverleafHttpError(
    `Overleaf request failed (${res.status}) for ${path11.basename(route)}: ${body.slice(0, 500)}`,
    res.status,
    code,
    body.slice(0, 500)
  );
...
var execFileAsync3 = (0, import_util14.promisify)(import_child_process3.execFile);
var SyncOwnerCoordinator = class {
  ...
  async claim(root, handler) {
    await this.release();
    this.root = await fs9.realpath(path12.resolve(root)).catch(() => path12.resolve(root));
    this.handler = handler;
    await fs9.mkdir(runtimeRoot(), { recursive: true, mode: 448 });
    await fs9.chmod(runtimeRoot(), 448).catch(() => void 0);
    const paths = runtimePaths(this.root);
    if (await canConnect(paths.socketPath)) return "client";
    await this.clearStale(paths);
    try {
      await fs9.mkdir(paths.lockPath, { mode: 448 });
    } catch (error) {
    ...
    try {
      await fs9.writeFile(paths.metadataPath, `${JSON.stringify(metadata, null, 2)}
`, { mode: 384 });
      await fs9.rm(paths.socketPath, { force: true });
      ...
      await fs9.chmod(paths.socketPath, 384);
      this.metadata = metadata;
      return "owner";
    } catch (error) {
      await fs9.rm(paths.lockPath, { recursive: true, force: true });
      throw error;
    }
  }
...
`;
  ...
  async subscribe(onEvent) {
    ...
    const subscribed = new Promise((resolve6, reject) => {
...
`);
  ...
  async handleSocketRequest(socket, value) {
    if (!isOwnerRequest(value) || !this.root || path12.resolve(value.root) !== path12.resolve(this.root)) {
...
function runtimePaths(root) {
  const hash = crypto4.createHash("sha256").update(path12.resolve(root)).digest("hex").slice(0, 32);
  const lockPath = path12.join(runtimeRoot(), `${hash}.lock`);
  return {
    lockPath,
    metadataPath: path12.join(lockPath, "owner.json"),
    ...
    socketPath: path12.join(runtimeRoot(), hash.slice(0, 16))
  };
}
...
function sendRequest(socketPath, request, timeoutMs) {
  return new Promise((resolve6, reject) => {
    ...
    const finish = (error, result) => {
      ...
      clearTimeout(timer);
      socket.destroy();
      error ? reject(error) : resolve6(result);
    };
...
async function processStartSignature(pid) {
  try {
    const result = await execFileAsync3("/bin/ps", ["-p", String(pid), "-o", "lstart="], { encoding: "utf8" });
...
function isOwnerRequest(value) {
  if (!value || typeof value !== "object") return false;
  const request = value;
  return request.version === 1 && typeof request.id === "string" && typeof request.command === "string" && typeof request.root === "string" && Boolean(request.args) && typeof request.args === "object";
}
function isOwnerEvent(value) {
  if (!value || typeof value !== "object") return false;
  const event = value;
...
// src/cli.ts
var execFileAsync4 = (0, import_util16.promisify)(import_child_process4.execFile);
...
function makeSuccessEnvelope(command, root, data, warnings) {
...
async function main(argv = process.argv.slice(2)) {
  ...
  try {
    ...
    output = new Output(json, command, root ? path13.resolve(root) : void 0);
    if (root) {
...
var WATCH_RESULT = /* @__PURE__ */ Symbol("watch");
async function dispatch(args, parsed, output, root) {
  ...
  if (group === "mirror" && action === "create") {
    const parent = path13.resolve(stringOption(parsed, "parent") ?? shared.localProjectsRoot);
  ...
  if (group === "config") {
    ...
    if (action === "set") {
      const key = operand;
      const value = args[3];
      if (!key || value === void 0) throw usageError("Use config set <key> <value>.");
  ...
  if (group === "pdf") {
    ...
    if (action === "open") {
      await execFileAsync4("/usr/bin/open", [pdf]);
...
var OwnerFacade = class {
  ...
  async getEngine() {
    ...
    const host = {
      ...
      conflict: (path14, reason) => this.coordinator.emit("conflict", { path: path14, reason })
    };
    ...
    while (!stopping) {
      ...
      if (role === "owner") {
        ...
        await new Promise((resolve6) => {
          activeSocket.once("close", resolve6);
          activeSocket.once("error", resolve6);
...
async function doctor(root, server, credentials, policy) {
  const identity = await credentials.getIdentity(server);
  const checks = {
    ...
    keychain: { ok: process.platform === "darwin" && await exists3("/usr/bin/security"), path: "/usr/bin/security" },
    authentication: { ok: Boolean(identity), server }
  };
  if (identity) {
    try {
      const projects = await new OverleafClient(server, identity, policy.networkTimeouts.httpMs / 1e3, policy.networkTimeouts).listProjects();
  ...
  if (root) {
    checks.manifest = { ok: await exists3(manifestPath(root)), path: manifestPath(root) };
    checks.owner = await inspectOwner(root);
  }
  return checks;
}
async function makeClient(serverUrl, credentials, policy) {
...
async function resolveMirrorRoot(candidate) {
  ...
  if (stat9?.isFile()) current = path13.dirname(current);
  while (true) {
...
async function operationIsDestructive(root, relPath, operation) {
...
function parseArgs(argv) {
  ...
  const valueOptions = /* @__PURE__ */ new Set(["root", "server", "parent", "use", "root-doc"]);
...
function stringOption(args, key) {
...
function getConfigValue(state, key) {
...
function setConfigValue(state, key, raw) {
  const allowed = /* @__PURE__ */ new Set([
    "serverUrl",
    "localProjectsRoot",
    "policy.autoPushLocalAhead",
    "policy.syncBinaryFiles",
    "policy.syncDestructiveChanges",
    "policy.networkTimeouts.connectMs",
    "policy.networkTimeouts.projectJoinMs",
    "policy.networkTimeouts.httpMs",
    "policy.networkTimeouts.joinDocMs",
    "policy.networkTimeouts.otAckMs"
  ]);
...
function commandName(positionals) {
  ...
  return (/* @__PURE__ */ new Set(["auth", "projects", "mirrors", "mirror", "config", "conflicts", "pdf"])).has(positionals[0]) ? positionals.slice(0, 2).join(" ") : positionals[0];
}
async function readSecret(prompt) {
  ...
  return new Promise((resolve6, reject) => {
    ...
    const onData = (chunk) => {
      ...
      } else if (chunk === "\x7F") value = value.slice(0, -1);
      else value += chunk;
    };
...
async function chooseProject(projects) {
  projects.forEach((project, index2) => process.stderr.write(`${index2 + 1}. ${project.name} \xB7 ${project.id}
`));
...
function usageError(message) {
  return new CliError(message, 3, "usage_error");
}
function authError(message) {
  return new CliError(message, 4, "authentication_error");
}
function dataError(message) {
  return new CliError(message, 7, "data_error");
}
