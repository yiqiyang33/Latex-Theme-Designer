% soulpos lives in collection-latexextra, so a BasicTeX or scheme-small install will not
% have it. Without this guard every document using the template would fail outright with
% "File `soulpos.sty' not found"; instead the inline highlights quietly fall back to the
% unbreakable chip, which is the same path CJK content already takes.
\newif\ifThemeInlineBreakable
\IfFileExists{soulpos.sty}{
    \RequirePackage{soul}
    \RequirePackage{soulpos}
    \RequirePackage{tikz}
    \ThemeInlineBreakabletrue
}{
    \ThemeInlineBreakablefalse
}
...
% soul re-typesets its argument token by token and cannot reconstruct CJK text under
% ctex/xeCJK -- it aborts the build with "Reconstruction failed". Such content falls back
% to the unbreakable chip: a failed build is worse than a highlight that will not wrap,
% and CJK has no inter-word spaces for soul to break at anyway.
\ExplSyntaxOn
\tl_new:N \l__theme_inline_tl
\NewDocumentCommand{\ThemeIfInlineSoulSafe}{m +m +m}{
    % The target has to be expanded into a token list first: passing
    % \tl_to_str:n {#1} inline leaves it unexpanded and the match then tests the
    % literal control sequence instead of the content.
    \tl_set:Nx \l__theme_inline_tl { \tl_to_str:n {#1} }
    \regex_match:nVTF { [^\x00-\x{2E7F}] } \l__theme_inline_tl {#3} {#2}
}
\ExplSyntaxOff
...
\NewDocumentCommand{\themeInlineBox}{O{} m m +m}{%
    ...
    \ifThemeInlineBreakable
        \ThemeIfInlineSoulSafe{#4}{%
            \kern\thmi@pad
            \begingroup\color{#3}#1\thmi@underlay{#4}\endgroup
            \kern\thmi@pad
        }{%
            \themeInlineChip[fontupper=#1]{#2}{#3}{#4}%
        }%
    \else
        \themeInlineChip[fontupper=#1]{#2}{#3}{#4}%
    \fi
